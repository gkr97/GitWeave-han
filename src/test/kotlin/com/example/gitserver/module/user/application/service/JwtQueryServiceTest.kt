...
class JwtQueryServiceTest {
    ...
    private val jwtQueryService = JwtQueryService(secretKey)
    ...
    fun `Bearer 토큰이 유효할 때 userId 리턴`() {
        ...
        // when
        val result = jwtQueryService.resolveUserIdByBearer(bearer)
    ...
    fun `토큰 값이 없으면 null 리턴`() {
        val result = jwtQueryService.resolveUserIdByBearer(null)
        assertEquals(null, result)
    }

    @Test
    fun `Bearer prefix가 아니면 null 리턴`() {
        val result = jwtQueryService.resolveUserIdByBearer("Basic abcd")
        assertEquals(null, result)
    }

    @Test
    fun `잘못된 JWT 토큰이면 null 리턴`() {
        val result = jwtQueryService.resolveUserIdByBearer("Bearer invalid.token.value")
        assertEquals(null, result)
    }
}
