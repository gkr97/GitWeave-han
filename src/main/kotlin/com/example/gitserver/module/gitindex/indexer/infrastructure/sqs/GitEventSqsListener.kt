                ...
                Thread.startVirtualThread {
                    try {
                        ...
                        LogContext.with(
                            ...
                            "repoId" to event.repositoryId.toString(),
                            "branch" to event.branch,
                            "userId" to (event.actorId ?: event.collaboratorUserId)?.toString()
                        ) {
                            when (event.eventType) {
                                ...
                                "PUSH" -> {
                                    ...
                                    log.info { "PUSH 이벤트 처리: repo=${event.name}, branch=${event.branch}, old=${event.oldrev}, new=${event.newrev}, actor=${event.actorId}" }
                                    indexingJobExecutor.indexRepository(event, bareRepoDir)
                                }
                                "BRANCH_CREATED" -> {
                                    log.info { "BRANCH_CREATED 처리: repo=${event.name}, branch=${event.branch}, head=${event.newrev}" }
                                }
                                "BRANCH_DELETED" -> {
                                    log.info { "BRANCH_DELETED 처리: repo=${event.name}, branch=${event.branch}" }
                                }
                                "REPO_RENAMED" -> {
                                    log.info { "REPO_RENAMED 처리: repoId=${event.repositoryId}, old=${event.oldName}, new=${event.newName}" }
                                }
                                "REPO_DELETED" -> {
                                    log.info { "REPO_DELETED 처리: repoId=${event.repositoryId}" }
                                }
                                "REPO_VISIBILITY_CHANGED" -> {
                                    log.info { "REPO_VISIBILITY_CHANGED 처리: repoId=${event.repositoryId}, visibility=${event.visibilityCode}" }
                                }
                                "COLLAB_INVITED", "COLLAB_ACCEPTED", "COLLAB_REJECTED", "COLLAB_REMOVED" -> {
                                    log.info { "COLLAB 이벤트 처리: type=${event.eventType}, repoId=${event.repositoryId}, user=${event.collaboratorUserId}" }
                                }
                                "REPO_STARRED", "REPO_UNSTARRED" -> {
                                    log.info { "REPO_STAR 이벤트 처리: type=${event.eventType}, repoId=${event.repositoryId}, user=${event.actorId}" }
                                }
                                else -> {
