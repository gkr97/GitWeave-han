    ...
    fun handleGit(
        ...
        when (access) {
            is RepositoryAccessQueryService.AccessResult.Authorized -> {
                val repoEntity = repositoryRepository.findByOwnerIdAndNameAndIsDeletedFalse(ownerId, repoName)
                if (repoEntity == null) {
                ...
                val isWrite = request.requestURI.contains("git-receive-pack")
                val redirected = if (isWrite) {
                    gitRouteRedirector.maybeRedirectWrite(repoEntity.id, request, response)
                } else {
                    gitRouteRedirector.maybeRedirectRead(repoEntity.id, request, response)
                }
